...
const chatbotProviders = [
  ...
  },
  {
    ...
    docsUrl: "https://nightbot.tv/dashboard",
    type: "json",
  },
  {
    id: "streamelements",
    ...
    docsUrl: "https://dashboard.streamelements.com/overlays",
    type: "json",
  },
] as const;
        ...
        <div className="py-8">
          ...
          {activeTab === "chatbot" && (() => {
            ...
            return (
              <div className="space-y-8">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.3em] text-purple-400">Chatbot</p>
                  <h1 className="mt-2 text-4xl font-black tracking-tighter text-white">Connectez votre bot</h1>
                </div>

                <div className="mx-auto max-w-4xl rounded-[36px] border border-zinc-800 bg-zinc-950/60 p-6 shadow-2xl shadow-black/30">
                  <div className="mb-6 flex justify-center">
                    <div className="inline-flex rounded-full border border-zinc-800 bg-zinc-950/80 p-1.5">
                  ...
                  <div className="rounded-[28px] border border-zinc-800 bg-zinc-900/70 p-6">
                    <div className="mb-8 rounded-[24px] border border-zinc-800 bg-black/20 p-5">
                      <p className="mb-3 text-[10px] font-black uppercase tracking-[0.25em] text-purple-300">Méthode recommandée</p>
                      <button
                        onClick={() => window.open(selectedProvider.docsUrl, "_blank", "noopener,noreferrer")}
                        className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 px-5 py-3 text-[10px] font-black uppercase tracking-[0.25em] text-white shadow-lg shadow-purple-600/20"
                      >
                    ...
                    <div className="mb-6 flex items-center justify-center text-[10px] font-black uppercase tracking-[0.25em] text-zinc-500">
                      <div className="h-px flex-1 bg-zinc-800" />
                      <span className="mx-3">Ou installation manuelle</span>
                      <div className="h-px flex-1 bg-zinc-800" />
                    </div>
                    ...
                    <div className="space-y-5">
                      <div className="rounded-[24px] border border-zinc-800 bg-black/20 p-4">
                        <div className="mb-3 flex items-center justify-between gap-3">
                          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Étape 1</span>
                          <button
                            onClick={() => handleCopyCommand(command)}
                            className="flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-800/80 px-3 py-2 text-[9px] font-black uppercase tracking-[0.2em] text-zinc-200"
                          >
                            <Copy size={12} />
                            {copiedCommand === command ? "Copié" : "Copier"}
                          </button>
                        </div>

                        <pre className="overflow-x-auto whitespace-pre-wrap break-all rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4 text-xs text-zinc-200">
                          {command}
                        </pre>
                      </div>

                      <div className="rounded-[20px] border border-zinc-800 bg-zinc-950/60 p-4 text-sm text-zinc-300">
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Étape 2</span>
                        <p className="mt-2">
                          Collez cette commande dans la section commandes personnalisées de {selectedProvider.name}, puis sauvegardez.
                        </p>
                      </div>

                      <div className="rounded-[20px] border border-zinc-800 bg-zinc-950/60 p-4 text-sm text-zinc-300">
                        <div className="mb-2 flex items-center justify-between gap-3">
                          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Message personnalisé</span>
                          <button
                            onClick={saveBotMessage}
                            disabled={loading}
                            className="rounded-xl border border-purple-500/30 bg-purple-500/10 px-3 py-2 text-[9px] font-black uppercase tracking-[0.2em] text-purple-200 transition hover:bg-purple-500/20 disabled:opacity-50"
                          >
                            Enregistrer
                          </button>
                        </div>
                        <textarea
                          ...
                          className="mt-3 w-full rounded-2xl border border-zinc-700 bg-black/20 p-3 text-sm text-zinc-100 outline-none focus:border-purple-500"
                          rows={3}
                        />
                        <p className="mt-2 text-[10px] uppercase tracking-[0.2em] text-zinc-500">Ce message s’applique à tous les bots en même temps.</p>
                        <p className="mt-1 text-[10px] uppercase tracking-[0.2em] text-zinc-500">Variables disponibles : {'{artist}'} {'{title}'} {'{song}'}</p>
                      </div>
                    </div>

                    <div className="mt-8 rounded-[24px] border border-zinc-800 bg-gradient-to-r from-zinc-900 via-zinc-950 to-black p-4 shadow-inner shadow-black/40">
                      <p className="mb-3 text-[10px] font-black uppercase tracking-[0.25em] text-zinc-500">Aperçu du chat</p>
                      <div className="rounded-2xl border border-zinc-700 bg-zinc-900/80 px-3 py-2">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-sm font-bold text-white">{previewText}</p>
                            <p className="text-[10px] uppercase tracking-[0.15em] text-zinc-400">Twitch Chat</p>
                          </div>
                        </div>
                      </div>
                    </div>
