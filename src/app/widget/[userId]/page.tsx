...
export default function SpotifyWidget() {
  ...
  const settings = {
    ...
    // Correction du Blur : on s'assure que c'est un nombre
  ...
  const fetchTrack = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Si data.isPlaying est false, on met à null pour l'animation de sortie
  ...
  return (
    <div className={`flex items-center justify-center min-h-screen bg-transparent ${settings.fontFamily}`}>
      <AnimatePresence mode="wait">
        {isVisible && (
          <motion.div 
            key="spotify-widget-fixed" // <--- IMPORTANT : Key fixe = Pas d'animation au changement de titre
          ...
          >
                ...
                <div className="absolute inset-0 z-0 overflow-hidden" style={{ borderRadius: `${settings.borderRadius}px` }}>
                  <div 
                    ...
                    style={{
                      ...
                      // Utilisation du blurAmount avec px
            ...
            {settings.showCover && (
              <motion.div 
                animate={{ x: settings.layout === 'minimal' ? 0 : -(cardWidth / 2) + 15 }}
                transition={sharedTransition}
